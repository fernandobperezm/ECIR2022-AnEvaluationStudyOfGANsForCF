...
All algorithms have the same basic interface, the constructor takes as input only sparse matrices
of scipy.sparse format.

All recommender take as first argument in the constructor the URM, content-based models also take
the ICM or UCM as the second.
- User Rating Matrix (URM) of shape |n users|x|n items| containing the user-item interactions,
  either implicit (1-0) or explicit (any value)
- Item Content Matrix (ICM) of shape |n items|x|n item features| containing the item features,
  again with any numerical value
- User Content Matrix (UCM) of shape |n users|x|n users features| containing the item features,
  again with any numerical value
...
from recsys_framework.Data_manager.Movielens.Movielens1MReader import Movielens1MReader
...
from recsys_framework.Recommenders.NonPersonalizedRecommender import TopPop
from recsys_framework.Recommenders.GraphBased.P3alphaRecommender import P3alphaRecommender
from recsys_framework.Recommenders.KNN.ItemKNNCBFRecommender import ItemKNNCBFRecommender
...
dataSplitter = DataSplitter_leave_k_out(
    data_reader, k_out_value=1, use_validation_set=True
)

# The load_data function will split the data and save it in the desired folder.
# Once the split is saved, further calls to the load_data will load the splitted data ensuring
...
# The constructor of the evaluator allows you to specify the evaluation conditions
# (data, recommendation list length, excluding already seen items). Whenever you want to evaluate
# a model, use the evaluateRecommender function of the evaluator object
