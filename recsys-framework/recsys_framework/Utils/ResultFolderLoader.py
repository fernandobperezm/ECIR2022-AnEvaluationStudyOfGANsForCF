...
from recsys_framework.Recommenders import *
from recsys_framework.Utils.decorators import log_calling_args
from recsys_framework.Utils.seconds_to_biggest_unit import seconds_to_biggest_unit
...
def _get_printable_recommender_name(RECOMMENDER_NAME):
    ...
    recommender_printable_name = (
        recommender_printable_name
            ...
            .replace(" NO MASK", "NM")
            .replace(" CODE HYPER PARAMETERS", " Code Params")
            .replace(" PAPER HYPER PARAMETERS", " Paper Params")
            .replace(" PAPER HYPER PARAMETERS", " Paper Params")
    )

    if "RandomNoise" in recommender_printable_name:
    ...
    if "ClassCondition" in recommender_printable_name:
        recommender_printable_name = recommender_printable_name.replace(
            "ClassCondition", ""
        ) + " CC"
    ...
    if len(KNN_similarity_list) == 0:
        for feature_matrix_name in feature_matrix_names_to_report_list:
            ...
            algorithm_data_to_print_list.append({
                "algorithm": algorithm,
                "algorithm_row_label": f"{algorithm_row_label} {feature_matrix_row_label}",
                "algorithm_file_name": f"{algorithm_file_name}_{feature_matrix_name}",
            })
    ...
    for similarity in KNN_similarity_list:
        for feature_matrix_name in feature_matrix_names_to_report_list:
            ...
            algorithm_data_to_print_list.append({
                ...
                # TODO: original value: "algorithm_file_name": algorithm_file_name + feature_matrix_name  + "_" + similarity,
                "algorithm_file_name": f"{algorithm_file_name}_{similarity}{feature_matrix_name}",
            })
...
class ResultFolderLoader(object):
    ...
    def get_results_dataframe(self,
        ...
        algorithm_label_index = [row_dict["algorithm_row_label"]
                                 ...
                                 else "algorithm_group_{}".format(row_index)
