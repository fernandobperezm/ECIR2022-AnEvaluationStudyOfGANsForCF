...
from recsys_framework.Evaluation.metrics import roc_auc, precision, precision_recall_min_denominator, recall, MAP, MRR, ndcg, arhr, \
...
class EvaluatorHoldout(Evaluator):
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size = None):
        ...
        if block_size is None:
            block_size = min(
                [# 1000,
                 # int(4 * 1e9 * 8 / 64 / self.n_items),
                 int(1 * 1e9 * 8 / 64 / self.n_items),
                 # int(1e8/self.n_items),
        ...
        # This while can be replaced with:
        #  np.array_split(ary=users_to_evaluate, indices_or_sections=len(users_to_evaluate) / block_size, axis=0)
...
class EvaluatorUserSubsetWrapper(object):
    ...
    def __init__(
        ...
        urm_validation_train: sps.csr_matrix
    ):
    ...
    def evaluateRecommender(
        ...
        recommender_object.set_URM_train(
            URM_train_new=self.urm_validation_train
        )

        results_dict, n_users_evaluated = self.evaluator_object.evaluateRecommender(
            recommender_object=recommender_object
        )

        # Restore the train URM
        recommender_object.set_URM_train(
            URM_train_new=urm_train_original
        )
