...
from recsys_framework.Data_manager.DataReader_utils import reconcile_mapper_with_removed_tokens, remove_features, \
    compute_num_rows_without_interactions
from recsys_framework.Data_manager.data_consistency_check import assert_URM_ICM_mapper_consistency
    ...
    def print_statistics(self):
        ...
        if self._HAS_ICM:
            ...
            for ICM_name, ICM_object in self.AVAILABLE_ICM.items():
                ...
                num_items_with_no_features = compute_num_rows_without_interactions(ICM_object)
                percentage_items_with_no_features = (num_items_with_no_features / n_items) * 100.0

                format_string = "2E" if np.max([np.abs(min_value), np.abs(max_value)])>100 else "2f"

                statistics_string = (
                    "\tICM name: {}, "
                    "Value range: {:.{format_string}} / {:.{format_string}}, "
                    "Num features: {}, "
                    "feature occurrences: {}, "
                    "density {:.2E}, "
                    "Items w/o features: {num_items_with_no_features}/{num_items} "
                    "({percentage_items_with_no_features:.2f}%)"
                ).format(
                    ICM_name,
                    min_value, max_value,
                    n_features,
                    ...
                    num_items_with_no_features=num_items_with_no_features,
                    num_items=n_items,
                    percentage_items_with_no_features=percentage_items_with_no_features,
                )
