...
from recsys_framework.Data_manager.IncrementalSparseMatrix import IncrementalSparseMatrix
import pandas as pd
...
def compute_num_rows_without_interactions(
    urm: sps.csr_matrix
) -> int:
    features_per_item: np.ndarray = urm.sum(
        axis=1,
        dtype=np.float32,
    ).A

    return features_per_item[
        np.isclose(features_per_item, 0.0)
    ].shape[0]
