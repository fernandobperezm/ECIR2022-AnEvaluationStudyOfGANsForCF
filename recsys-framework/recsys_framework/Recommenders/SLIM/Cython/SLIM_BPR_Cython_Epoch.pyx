...
from recsys_framework.Recommenders.Recommender_utils import similarityMatrixTopK, check_matrix
import cython
import time
import sys
    ...
    def epochIteration_Cython(self):
        ...
        for numCurrentBatch in range(totalNumberOfBatch):
            ...
            if self.verbose and ((numCurrentBatch%printStep==0 and not numCurrentBatch==0) or numCurrentBatch==totalNumberOfBatch-1):
                print("Cython-Processed {} ( {:.2f}% ) in {:.2f} seconds. BPR loss is {:.2E}. Sample per second: {:.0f}".format(
