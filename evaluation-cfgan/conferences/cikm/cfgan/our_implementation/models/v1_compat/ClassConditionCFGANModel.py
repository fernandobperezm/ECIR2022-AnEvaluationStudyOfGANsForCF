...
class ClassConditionCFGANModel(CFGANModelAbstract):
    def __init__(
        self,
    ...
    def _calculate_generator_discriminator_inputs_outputs(
        ...
        generator_num_input_features = self._num_rows_dataset
        generator_num_output_features = self._num_cols_dataset
        discriminator_num_input_features = self._num_rows_dataset + self._num_cols_dataset
    ...
    def _get_condition(
        self,
        class_indices: tf.compat.v1.Tensor,
    ) -> tf.compat.v1.Tensor:
        """
        Parameters
        ----------
        class_indices
         tensor containing the row identifiers to apply one-hot encoding.
        """
        num_classes = self._num_rows_dataset
        # The one_hot function works in this way: in 'indices' you pass the values that you want to one_hot encode
        # and in 'depth' you pass the number of total unique values that your dataset has. The 'on_value' parameter
        # will place that value when the class matches, else it will put 'off_value'
        return tf.compat.v1.dtypes.cast(
            x=tf.compat.v1.one_hot(
    ...
    def _get_discriminator_fake_and_real_input(
        ...
        mask: tf.compat.v1.Tensor
        if self.hyper_parameters.mask_type == CFGANMaskType.NO_MASK:
            mask = tf.convert_to_tensor(1.0, dtype=tf.float32)
        elif self.hyper_parameters.mask_type == CFGANMaskType.ZERO_RECONSTRUCTION:
    ...
    def _get_generator_and_discriminator_names(
        ...
        generator_model_name = (
            f"v1_compat_ClassConditionCFGAN_"
    ...
    def get_item_weights(
        ...
        **kwargs,
        ...
        item_weights: np.ndarray = self.sess.run(
            ...
            feed_dict={
                self.ph_condition_class_indices: class_indices
            }
        )
        ...
        for batch_id in range(num_of_mini_batches):
            ...
            (
                ...
                feed_dict={
                    ...
                    self.ph_condition_class_indices: batch_indices,
                },
            )
