...
from conferences.cikm.cfgan.our_implementation.parameters import CFGANHyperParameters, RandomNoiseCFGANHyperParameters
...
class RandomNoiseCFGANModel(CFGANModelAbstract):
    def __init__(
        self,
        urm_train: sp.csr_matrix,
        hyper_parameters: RandomNoiseCFGANHyperParameters,
        ...
        with tf.compat.v1.Graph().as_default():
            ...
            self.ph_random_noise = tf.compat.v1.placeholder(
                dtype=tf.compat.v1.float32,
                shape=(None, self.hyper_parameters.noise_size),
                name="ph_random_noise"
            )
            ...
            generator_input = self._get_generator_input(
                random_noise=self.ph_random_noise,
    ...
    def _calculate_generator_discriminator_inputs_outputs(
        ...
        generator_num_input_features = self._num_cols_dataset + self.hyper_parameters.noise_size
    ...
    def _get_generator_input(
        self,
        random_noise: tf.compat.v1.Tensor,
        condition: tf.compat.v1.Tensor,
        **kwargs,
    ) -> tf.compat.v1.Tensor:
        return tf.compat.v1.concat(
            values=[condition, random_noise],
            axis=1
        )
    ...
    def _get_generator_random_noise(
        self,
        batch_size: int,
    ) -> np.ndarray:
        """
        ...
        return np.random.normal(
            loc=0.0,
            scale=1.0,
            size=(batch_size, self.hyper_parameters.noise_size),
    ...
    def _train_step_discriminator(
        ...
        for batch_id in range(num_of_mini_batches):
            ...
            random_noise = self._get_generator_random_noise(
            ...
            (
                ...
                feed_dict={
                    ...
                    self.ph_random_noise: random_noise,
    ...
    def get_item_weights(
        ...
        **kwargs,
        ...
        random_noise = self._get_generator_random_noise(
            batch_size=self._num_rows_dataset
        )
    ...
    def load_model(
        ...
        super()._load_model(
            ...
            file_name=file_name,
            hyper_parameter_cls=RandomNoiseCFGANHyperParameters,
