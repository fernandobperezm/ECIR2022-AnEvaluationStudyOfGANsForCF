...
def FullyConnectedLayer(
    input, inputDim, outputDim, activation, model, layer, reuse=False
):
    ...
    with tf.variable_scope(model) as scope:
        ...
        b = tf.get_variable(
            bName, [outputDim], initializer=tf.random_uniform_initializer(-0.01, 0.01)
        )
...
def GEN(input, userCount, h, activation, hiddenLayers):
    ...
    # hidden -> output
    y, this_L2, W, b = FullyConnectedLayer(
        y, h, userCount, "none", "gen", hiddenLayers + 1
    )
    L2norm = L2norm + this_L2

    # loss function for ZR
    ZR_loss = tf.reduce_mean(
        tf.reduce_sum(tf.square(y - 0) * ZR_dims, 1, keep_dims=True)
    )
...
def DIS(input, inputDim, h, activation, hiddenLayers, _reuse=False):
    # input->hidden
    y, _, W, b = FullyConnectedLayer(
        input, inputDim, h, activation, "dis", 0, reuse=_reuse
    )

    # stacked hidden layers
    for layer in range(hiddenLayers - 1):
        y, _, W, b = FullyConnectedLayer(
            y, h, h, activation, "dis", layer + 1, reuse=_reuse
        )

    # hidden -> output
    y, _, W, b = FullyConnectedLayer(
        y, h, 1, "none", "dis", hiddenLayers + 1, reuse=_reuse
    )
